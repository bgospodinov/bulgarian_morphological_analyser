...
# example  . predictions/compute_pred_ud.sh predictions/ud/bg-dev-pred.txt data/UD_Bulgarian-BTB/bg-ud-dev.conllu.conv > predictions/ud/bg-dev-res.txt
# example  . predictions/compute_pred_ud.sh predictions/ud/bg-test-pred.txt data/UD_Bulgarian-BTB/bg-ud-test.conllu.conv > predictions/ud/bg-test-res.txt
# DO NOT USE - BROKEN
...
# morph accuracy
morph_wrong=`diff --strip-trailing-cr -U 0 <(cut -d$'\t' -f6 $1) <(cat $2 | tr -s ' ' | cut -d' ' -f3) | grep -c ^@`
